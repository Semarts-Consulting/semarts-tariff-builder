...
| Task | Owner | Files | Dependencies | Risk | Notes |
| --- | --- | --- | --- | --- | --- |
| ASSET-002 Asset valuation methodology decision pack | PM plus Tariff Engine review | `docs/ASSET_VALUATION_METHOD_DECISION_PACK.md`, manager docs | WB-005 asset evidence and decision pack | High | Defines when asset evidence may calculate or become approved annual asset recovery cost pools without changing tariff engine semantics. |
...
| CTRL-010 Current evidence closeout | PM | `docs/OVERNIGHT_REVIEW_AND_NEXT_PHASE_PLAN.md`, `docs/MVP_TASK_BOARD.md`, `docs/PM_CONTROL.md`, `docs/LONG_RUN_PROGRESS_LOG.md` | Current branch package | Low | Records current baseline after PR #69 and confirms the next phase should use decision packs before tariff-impacting implementation. |
...
| PR-052 Workbook-derived scenario plan | PM plus QA/Tariff Engine/Data Import review | `docs/WORKBOOK_DERIVED_SCENARIO_PLAN.md`, manager docs | Merged PR #52 | Medium | Representative workbook-derived scenario backlog recorded; no fixtures, tests, imports, calculations, storage, reports, exports, shared DTOs, or UI changes approved by the plan alone. |
| WB-001 Airport customer-class scenario regression | QA plus Tariff Engine/Data Import/PM review | `docs/WB_001_FIXTURE_IMPLEMENTATION_PROPOSAL.md`, `tests/fixtures/workbook-derived-scenarios.ts`, `tests/workbook-derived-scenarios.test.ts`, `docs/LONG_RUN_PROGRESS_LOG.md` | Long-run validation: lint, type-check, 19 test files and 121 tests, build passed | Medium | Test-only airport customer-class scenario implemented without production calculation, import, storage, report, export, shared DTO, or UI behaviour changes. |
| WB-006 Weak mapping confidence regression | QA plus Data Import/PM review | `docs/WB_006_WEAK_MAPPING_CONFIDENCE_SCENARIO.md`, `tests/fixtures/workbook-derived-scenarios.ts`, `tests/workbook-derived-scenarios.test.ts`, `docs/LONG_RUN_PROGRESS_LOG.md` | Long-run validation: lint, type-check, 19 test files and 122 tests, build passed | Medium | Test-only coverage proves weak, unresolved, evidence-only, and manual-review workbook mappings stay outside tariff inputs. |
| REPORT-003 Non-ready report HTML regression | QA plus UI/PM review | `tests/report-readiness.test.tsx`, `docs/LONG_RUN_PROGRESS_LOG.md` | Long-run validation: lint, type-check, 19 test files and 123 tests, build passed | Low | Downloaded HTML coverage now includes non-ready readiness issues, revenue variance, and validation messages. |
| WB-002 TLM/local losses evidence regression | QA plus Tariff Engine/PM review | `docs/WB_002_TLM_LOCAL_LOSSES_SCENARIO.md`, `tests/fixtures/workbook-derived-scenarios.ts`, `tests/workbook-derived-scenarios.test.ts`, `docs/LONG_RUN_PROGRESS_LOG.md` | Focused workbook-derived scenario test passed; lint and type-check passed | High | Test-only coverage proves loss-adjusted evidence remains outside tariff volume inputs until loss methodology is approved. |
| WB-004 Generation/export evidence regression | QA plus Tariff Engine/PM review | `docs/WB_004_GENERATION_EXPORT_EVIDENCE_SCENARIO.md`, `tests/fixtures/workbook-derived-scenarios.ts`, `tests/workbook-derived-scenarios.test.ts`, `docs/LONG_RUN_PROGRESS_LOG.md` | Long-run validation: lint, type-check, 19 test files and 125 tests, build passed | High | Test-only coverage proves generation/export volumes and credit evidence do not net consumption, reduce recoverable cost, or change revenue requirement. |
| WB-003 Port tenant recovery forecast regression | QA plus Data Import/Tariff Engine/PM review | `docs/WB_003_PORT_TENANT_RECOVERY_SCENARIO.md`, `tests/fixtures/workbook-derived-scenarios.ts`, `tests/workbook-derived-scenarios.test.ts`, `docs/LONG_RUN_PROGRESS_LOG.md` | Long-run validation: lint, type-check, 19 test files and 126 tests, build passed | High | Test-only coverage proves tenant recovery forecasts do not automatically become tariff customer classes, cost pools, or revenue requirements. |
| WB-005 Asset-cost allocation regression | QA plus Tariff Engine/Data Import/PM review | `docs/WB_005_ASSET_ALLOCATION_SCENARIO.md`, `docs/WB_005_ASSET_DECISION_PACK.md`, `tests/fixtures/workbook-derived-scenarios.ts`, `tests/workbook-derived-scenarios.test.ts` | Focused workbook-derived scenario test pending | Medium | Option A approved. Test-only coverage uses pre-set annual asset amounts and does not introduce production asset valuation, import, storage, shared DTO, report, export, UI, or calculation behaviour changes. |
