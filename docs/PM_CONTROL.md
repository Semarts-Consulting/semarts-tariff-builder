...
- `docs/TARIFF_MODEL_YEAR_CONTRACT_PROPOSAL.md` proposes the first non-binding contract for long-lived tariff models and annual tariff years. It does not approve storage migration, route changes, shared DTO changes, project renaming, calculation changes, imports, exports or report total changes.
- `docs/INPUT_SELECTION_DATA_MODEL_PROPOSAL.md` proposes the future selection-record pattern for tariff-year inputs. It requires explicit tariff-use state so evidence-only UtilityHub data cannot silently become tariff-driving.
...
- UtilityHub PR #2 has merged the Tariff Builder selector contract baseline in `docs/TARIFF_SELECTOR_CONTRACT.md` and `src/services/tariff-selector-contract.ts`. This clears the first contract blocker for customer/site selector planning, but does not approve live API integration, storage migration, automatic tariff-year input population, raw meter reading ingestion, tariff-impacting selected data, calculation changes, report total changes, export changes or shared DTO changes.
...
- User test after PR #98 is accepted. The first-release walkthrough evidence now records the current baseline as accepted for controlled walkthrough; the next PM action is a release decision record rather than another manual walkthrough.
- Live UtilityHub selector integration consumes UtilityHub PR #11 local selector envelopes when live mode and base URL are configured. New Project can load customer/site options from UtilityHub, but selected records remain evidence-only; no persistence of selected input records or tariff-driving conversion is approved.
...
Use `docs/UTILITYHUB_DATA_OWNERSHIP_AND_TARIFF_YEAR_MODEL.md`, `docs/FIRST_RELEASE_READINESS_REVIEW.md`, `docs/FIRST_RELEASE_WALKTHROUGH_EVIDENCE.md`, `docs/LOCAL_ONLY_RUN_MODE.md`, `docs/LIVE_UTILITYHUB_SELECTOR_INTEGRATION.md` and selector boundary docs as the current release-readiness references. The next safe package after user testing is either selected-input persistence design or a controlled release decision record. Do not start tariff-driving conversion, production methodology, import parser contract changes, storage migration, report total changes, export DTO changes, shared DTO changes, methodology-derived cost-pool generation, asset valuation, UtilityHub data sync beyond selectors, CPI source integration or calculation behaviour changes until the relevant decision pack and shared UtilityHub contracts are approved.
