...
Use `docs/UTILITYHUB_DATA_OWNERSHIP_AND_TARIFF_YEAR_MODEL.md`, `docs/FIRST_RELEASE_READINESS_REVIEW.md`, `docs/FIRST_RELEASE_WALKTHROUGH_EVIDENCE.md`, `docs/LOCAL_ONLY_RUN_MODE.md` and selector boundary docs as the current release-readiness references. The next safe package is a controlled release decision record. Do not start live UtilityHub fetches, selected-input persistence, production methodology, import parser contract changes, storage, report total, export, shared DTO, UtilityHub hierarchy, Meter Map consumption, methodology-derived cost-pool generation, asset valuation, UtilityHub data sync, CPI source integration or calculation behaviour changes until the relevant decision pack and shared UtilityHub contracts are approved.
